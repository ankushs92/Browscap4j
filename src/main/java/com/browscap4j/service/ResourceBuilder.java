...
public final class ResourceBuilder {
	...
	public Map<String,String> getRegexNamePatternsMap() {
		...
		//The NamePatterns should be first sorted based on ascending order of length.
		return records.stream()
			   .collect(Collectors.toMap(record -> record[0], 
					   					record -> RegexResolver.toRegex(record[0])))
			   .entrySet()
			   .stream()
